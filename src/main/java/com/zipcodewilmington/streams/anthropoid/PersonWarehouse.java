...
public final class PersonWarehouse implements Iterable<Person> {
    ...
    public List<String> getNames() {
        return Stream.of(people).flatMap(Collection::stream).map(Person::getName).collect(Collectors.toList());
    }
    ...
    public Map<Long, String> getIdToNameMap() {
        ...
        people.stream().forEach(p -> result.put(p.getPersonalId(),p.getName()));
        return result;
    }
